 ...
 * ############################
 * Program notes:
 * The capactitance (moisture content) is measured in frequency of the oscillator but counting the number of
 *  high pulses in 1 second.
 *
 * The temperature is measured using an IC that  outputs a square wave, so the pulseIn function is used
 * to measure positive and negative levels of the waveform and calculate the frequency.
 *
 */
...
int one_second_flag = 0; //flags when the 1second timer has elapsed
...
void SetContinueFlag(){
  ...
  //RGB.color(255,255,0);
  one_second_flag = 1; //set flag // sets flag to finish program
}

void CapISR(void){  //interrupt routine called every time a pulse is detected
//  RGB.color(0,0,255);
  if(!timer.isActive()){ //starts 1 second timer is it hasn't already started
    timer.startFromISR();
  }
...
/*
* #######################################
* This if-else statement activates the sensor circuitry and activates the pulse count interrupt
*/
  if(LoopFlag == 1){
    //RGB.color(255,0,0);
  ...
  else if(one_second_flag == 1){ //if 1 second timer finished (pulse count routine finished)
    //RGB.color(0,255,0);
    // Toggles Second Flag
    one_second_flag = 0;
    LoopFlag = 1;

    CapFreq = PulseCount; //pulses in 1 second = frequency
    PulseCount = 0;
